...
/**
 * @description: 获取格式化后的时区列表
 */
export const getTimezoneList = function () {
  // zonename :  "America/New York"  timezone : "EST5EDT,M3.2.0,M11.1.0"
  return TIMEZONE.map((arr) => ({ value: arr.join(SEPARATOR), label: arr[0] }));
};

export const TIMEZONE_LIST = getTimezoneList();

/**
 * @description: 将时区按固定字符拆分为数组
 */
export const getZoneNameAndTimezone = function (value: string) {
  if (!value) return ["", ""];
 ...
 * @return 拼接后的字符串
 */
export const joinZoneNameAndTimezone = function (zonename: string, timezone: string) {
  return `${zonename}${SEPARATOR}${timezone}`;
};
